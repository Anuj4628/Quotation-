...
export const ProformaDetailPage: React.FC = () => {
  ...
  const [newStatus, setNewStatus] = useState<ProformaStatus>('approved');
  const [statusNote, setStatusNote] = useState('');
  const [isGeneratingPdf, setIsGeneratingPdf] = useState(false);

  const proforma = useMemo(() => {
    if (!id) return undefined;
    return storage.getProformaInvoiceById(id);
  }, [id, refreshKey]);

  if (!proforma) {
    return (
      <div className="py-20 text-center">
        <h2 className="text-xl font-bold text-slate-800">Proforma Invoice Not Found</h2>
  ...
  const handleDuplicate = () => {
    const dup = storage.duplicateProformaInvoice(proforma.id);
    if (dup) {
      success('Proforma Duplicated', `Created new draft ${dup.proformaNumber}`);
      navigate(`/proformas/${dup.id}`);
    }
  };

  const handleDelete = () => {
    if (window.confirm(`Are you sure you want to delete Proforma Invoice ${proforma.proformaNumber}?`)) {
      storage.deleteProformaInvoice(proforma.id);
      success('Proforma Deleted', `Deleted ${proforma.proformaNumber}`);
      navigate('/proformas');
    }
  };
  ...
  const handleDownloadPdf = async () => {
    ...
    if (ok) {
      success('PDF Generated', `Downloaded ${proforma.proformaNumber}.pdf`);
    }
  };
  ...
  const handleStatusChangeSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    storage.updateProformaInvoiceStatus(proforma.id, newStatus, statusNote);
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="no-print bg-white p-4 sm:p-6 rounded-2xl border border-slate-200 shadow-soft flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <span
                className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-bold border uppercase tracking-wider ${
                  statusBadgeClasses[proforma.status] || 'bg-slate-100 text-slate-700'
                }`}
              >
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Buyer: <strong className="text-slate-800">{proforma.customerName}</strong> • Consignee: <strong className="text-slate-800">{proforma.shipToCompany || proforma.customerName}</strong> • Date: {proforma.proformaDate}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={() => navigate(`/proformas/${proforma.id}/edit`)}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold transition-colors"
          >
      ...
      <div className="bg-slate-200/50 p-2 sm:p-6 rounded-2xl border border-slate-300/80 overflow-x-auto print:bg-white print:border-none print:p-0">
        <ProformaDocument proforma={proforma} id="proforma-print-document" />
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-900">Change Proforma Status</h3>
              <button
